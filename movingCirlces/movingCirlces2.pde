...
class Circle{
  ...
  public Circle(int x, int y){
    ...
    this.offsetX = 0;
    this.offsetY = 0;
    this.random = (int) random(2048);    
  }
  
}

  Circle[] circles = new Circle[numberOfCirlces];
void setup(){
  size(750,500);
...
void draw(){
  ...
  for(int i = 0; i < numberOfCirlces; i++){
    ...
    circles[i].currX = (int)(circles[i].x + distanceOfCirlces*cos(t + circles[i].random));
    circles[i].currY = (int)(circles[i].y + distanceOfCirlces*sin(t + circles[i].random));
